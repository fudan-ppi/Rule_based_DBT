...
# 141 "odoc_see_lexer.ml"

  | 2 ->
# 38 "odoc_see_lexer.mll"
      (
        print_DEBUG2 "call url lexbuf" ;
        url lexbuf
      )
# 149 "odoc_see_lexer.ml"

  | 3 ->
# 44 "odoc_see_lexer.mll"
      (
        print_DEBUG2 "call doc lexbuf" ;
        doc lexbuf
      )
# 157 "odoc_see_lexer.ml"

  | 4 ->
# 51 "odoc_see_lexer.mll"
      (
        print_DEBUG2 "call file lexbuf" ;
        file lexbuf
      )
# 165 "odoc_see_lexer.ml"

  | 5 ->
# 57 "odoc_see_lexer.mll"
      (
        print_DEBUG2 "EOF";
        EOF
      )
# 173 "odoc_see_lexer.ml"

  | 6 ->
# 63 "odoc_see_lexer.mll"
      (
        Buffer.reset buf ;
        Buffer.add_string buf (Lexing.lexeme lexbuf);
        desc lexbuf
      )
# 182 "odoc_see_lexer.ml"
...
and __ocaml_lex_doc_rec lexbuf __ocaml_lex_state =
  match Lexing.engine __ocaml_lex_tables __ocaml_lex_state lexbuf with
      | 0 ->
# 80 "odoc_see_lexer.mll"
      (
        let s = Lexing.lexeme lexbuf in
        See_doc (String.sub s 0 ((String.length s) -1))
      )
# 211 "odoc_see_lexer.ml"
...
and __ocaml_lex_file_rec lexbuf __ocaml_lex_state =
  match Lexing.engine __ocaml_lex_tables __ocaml_lex_state lexbuf with
      | 0 ->
# 87 "odoc_see_lexer.mll"
      (
        let s = Lexing.lexeme lexbuf in
        See_file (String.sub s 0 ((String.length s) -1))
      )
# 225 "odoc_see_lexer.ml"
...
and __ocaml_lex_desc_rec lexbuf __ocaml_lex_state =
  match Lexing.engine __ocaml_lex_tables __ocaml_lex_state lexbuf with
      | 0 ->
# 95 "odoc_see_lexer.mll"
      ( Desc (Buffer.contents buf) )
# 236 "odoc_see_lexer.ml"
