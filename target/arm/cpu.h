...
#define FFR_PRED_NUM 16
   ...
   uint32_t cc_source; /* source of condition code,
							  0: TCG, 1: Host(rule translation) */
   uint16_t x86_eflags; /* saved host eflags */
	
   #ifdef PROFILE_RULE_TRANSLATION
	   uint64_t total_dynamic_insts;
	   uint64_t rule_dynamic_insts;
   #endif
	
   #ifdef PROFILE_TB_HOTNESS
	   uint64_t total_dynamic_tbs;
   #endif
	
   #ifdef PROFILE_INDIRECT_BRANCH
	   uint64_t indirect_dynamic_insts;
   #endif	
} CPUARMState;
