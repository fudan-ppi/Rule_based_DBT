...
{
  disassembler_ftype disas = disassembler(bfd_get_arch(prog->abfd), bfd_big_endian(prog->abfd), bfd_get_mach(prog->abfd), prog->abfd);
...
char* string_of_insn(asm_program_t *prog, Instruction *inst)
{
  ...
  char *old_opts = (char *)prog->disasm_info.disassembler_options;
