...
class virtual fragment_machine = object
  ...
  method virtual symbolic_word_reg : register_name -> Vine.exp
  method virtual symbolic_long_reg : register_name -> Vine.exp
  method virtual symbolic_word_var : Vine.var -> Vine.exp
  ...
  method virtual set_mem_symbolic : 
    (register_name * string * int) -> string -> Vine.typ -> Vine.stmt
  method virtual get_symbolic_mem : Vine.exp -> Vine.typ -> Vine.exp

  method virtual get_reg_exp : register_name -> Vine.exp

  method virtual generate_fresh_var : string -> Vine.typ -> Vine.var
  method virtual generate_exp_with_fresh_symbolic : string -> Vine.exp
  method virtual generate_initialized_var : string -> Vine.typ -> Vine.var
  method virtual declare_initialized_var : Vine.var -> string -> unit

  method virtual set_word_reg_constant : register_name -> int -> unit
...
(* 	self#store_byte_conc 0x60000037L 0xff; (* base high *) *)
  ...
  reg R_VF (D.from_concrete_1  0);
    ...
    method symbolic_word_var var = 
      D.to_symbolic_32 (self#get_int_var var)

    method symbolic_word_reg r =
      D.to_symbolic_32 (self#get_int_var (Hashtbl.find reg_to_var r))

    method symbolic_long_reg r =
      D.to_symbolic_64 (self#get_int_var (Hashtbl.find reg_to_var r))
    ...
    (* Currently, we only support base+offset addressing mode *)
    method private get_addr_exp mem_i =
      let (reg, idx, off) = mem_i in
      let reg_var = Hashtbl.find reg_to_var reg in
      let reg_exp = V.Lval(V.Temp(reg_var)) in
      let off_exp = V.Constant(V.Int(V.REG_32, (Int64.of_int off))) in
      let addr_exp = V.BinOp(V.PLUS, reg_exp, off_exp) in
      if idx <> "none" then
        let idx_reg = regstr_to_reg idx in
        let idx_var = Hashtbl.find reg_to_var idx_reg in
        let idx_exp = V.Lval(V.Temp(idx_var)) in
        let addr2_exp = V.BinOp(V.PLUS, addr_exp, idx_exp) in
        addr2_exp
      else
        addr_exp

    method private generate_var_symbolic name typ = 
      let var = V.newvar name typ in
      V.VarHash.add reg_store var (D.uninit);
      self#set_int_var var (form_man#fresh_symbolic_32 name);
      var

    method set_mem_symbolic mem_i name typ =
      let addr_exp = self#get_addr_exp mem_i in
      let mem_exp = V.Mem(mem_var, addr_exp, typ) in
      let init_var = self#generate_var_symbolic name typ in 
      let var_exp = V.Lval(V.Temp(init_var)) in
      let st = V.Move(mem_exp, var_exp) in
      st
    
    method get_symbolic_mem mem_addr typ =
      (*let addr_exp = self#get_addr_exp mem_i in*)
      let (v, _) = self#handle_load mem_addr typ in
      let sv = D.to_symbolic_32 v in
      sv

    method get_reg_exp reg =
      let reg_var = Hashtbl.find reg_to_var reg in
      let reg_exp = V.Lval(V.Temp(reg_var)) in
      reg_exp

    method generate_fresh_var name typ =
      let var = V.newvar name typ in
      V.VarHash.add reg_store var (D.uninit);
      var

    method generate_exp_with_fresh_symbolic s =
      let var = self#generate_var_symbolic s V.REG_32 in
      let exp = V.Lval(V.Temp(var)) in
      exp

    method generate_initialized_var name typ =
      let var = V.newvar name typ in
      V.VarHash.add reg_store var (D.uninit);
      self#set_int_var var (form_man#fresh_symbolic_32 name);
      var

    method declare_initialized_var var name =
      V.VarHash.add reg_store var (D.uninit);
      self#set_int_var var (form_man#fresh_symbolic_32 name);

    method set_word_reg_constant reg value =
      self#set_int_var (Hashtbl.find reg_to_var reg)
        (D.from_concrete_32 (Int64.of_int value));
    ...
    (*Printf.printf "==== I am executing: %s\n" (stmt_to_string_compact st);*)
	      stmt_num <- stmt_num + 1;
	      (match st with
