...
void arm_translate_init(void)
{
	...
	cpu_CC_Source = tcg_global_mem_new_i32(cpu_env, offsetof(CPUARMState, cc_source), "CC_SOURCE");
                                       ...
                                       TCGv_i32 var, ARMOperand *opd)
{
    ...
    if (!(insn & (1 << 25))) {
        ...
        if (!(insn & (1 << 23)))
            val = -val;
		set_arm_opd_mem_off_val(opd, val);
        if (val != 0)
            tcg_gen_addi_i32(var, var, val);
    } else {
        /* shift/register */
        rm = (insn) & 0xf;
		set_arm_opd_mem_index_reg(opd, rm);
        shift = (insn >> 7) & 0x1f;
        shiftop = (insn >> 5) & 3;
		set_arm_opd_mem_scale_imm(opd, shiftop, shift);
        offset = load_reg(s, rm);
        gen_arm_shift_im(offset, shiftop, shift, 0);
        if (!(insn & (1 << 23))){
            tcg_gen_sub_i32(var, var, offset);
			set_arm_opd_mem_addr_op(opd, ARM_MEM_ADDR_OP_TYPE_SUB);
        	}
        else
...
static inline void gen_add_datah_offset(DisasContext *s, unsigned int insn,
                                        int extra, TCGv_i32 var, ARMOperand *opd)
{
    int val, rm;
    TCGv_i32 offset;
    ...
    if (insn & (1 << 22)) {
		...
		//set_arm_opd_mem_off_val(opd, extra);
        rm = (insn) & 0xf;
		set_arm_opd_mem_index_reg(opd, rm);
        offset = load_reg(s, rm);
        if (!(insn & (1 << 23)))
...
/*...*/
static bool insn_define_cc(unsigned int insn)
{
    unsigned int op1;
    ...
    } else if ((insn & 0x00000090) == 0x90) {
        op1 = (insn >> 24) & 0xf;
        if (op1 == 0x0) {
            op1 = (insn >> 20) & 0xf;
            if (op1 == 0 || op1 == 1 || op1 == 2 ||
                op1 == 3 || op1 == 6 || /* 32 bit mul, mul add, mul sub */
                op1 == 8 || op1 == 9 || op1 == 10 ||
                op1 == 11 || op1 == 12 || op1 == 13 ||
                op1 == 14 || op1 == 15 /* 64 bit mul: UMULL, UMLAL, SMULL, SMLAL. */) {
                /* 32 bit mul, mul add, mul sub */
                if (insn & (1 << 20))
                    return true;
            }
        }
...
/* 1: bb at pc_start uses cc before defines it, but OF is not used
   2: bb at pc_start uses cc before defines it, and OF is used
   3: bb at pc_start defines cc before uses it */
static int tb_define_cc(DisasContext *s, target_ulong pc_start)
{
    target_ulong pc = pc_start;
    unsigned int insn, cond, op1;
    int ret = 0;
    int ret2 = 3;

//if (s->tb->pc == 0x409baa10) {
//   fprintf(stderr, "==================fsaf==\n");
//    sleep(20);
//}

    while (1) {
        insn = arm_ldl_code(s->env, pc, s->sctlr_b);
        pc += 4;

        /* conditional execution use cc */
        cond = insn >> 28;
        if (cond != 0xe) {
            if (ret != 2)
                ret = (cond == 4 || cond == 10 || cond == 11 || cond == 12 || cond == 13) ? 2 : 1;
        }

        /* branch (and link) or swi */
        op1 = (insn >> 24) & 0xf;
        if (op1 == 0xa || op1 == 0xb) { /* branch */
            int32_t target = (int32_t)pc + sextract32(insn << 2, 0, 26) + 4;
            //fprintf(stderr, "tb->pc: %x  cur pc: %x target: %x\n", s->tb->pc, pc - 4, target);
            if (cond == 0xe && pc - 4 != target) 
                ret2 = tb_define_cc(s, target);
            goto update_ret;
        } else if (op1 == 0xf) { /* swi */
            goto update_ret;
        } else if (op1 == 0xb) { /* branch link */
            ret2 = 2;
            goto update_ret;
        } else if (op1 == 0x08 || op1 == 0x09) { /* load/store multiple words */
            bool is_load = extract32(insn, 20, 1);
            if (is_load && (insn & (1 << 15))) { /* load to pc(r15) */
                ret2 = 2;
                goto update_ret;
            }
        }

        /* write pc(r15) register */
        if (((insn & 0x0f900000) == 0x03000000 &&
            (insn & (1 << 21)) == 0 && ((insn >> 12) & 0xf) == 15) || 
            /* Hack */
            insn == 0xe240f01f || insn == 0xe5bcff98 || insn == 0xe5bcf560 ||
            insn == 0xe5bcf284 || insn == 0xe5bcf22c || insn == 0xe5bcf500 ||
            insn == 0xe5bcf4d0 || insn == 0xe5bcf310 || insn == 0xe5bcfd8c ||
            insn == 0xe5bcfec8 || insn == 0xe5bcfe68 || ((insn & 0xe5bcf000) == 0xe5bcf000)) {
            ret2 = 2;
            goto update_ret;
        }
        ...
        if ((insn & 0x0f900000) == 0x01000000 &&
            ...
            op1 = (insn >> 21) & 3;
            if ((sh == 0x1) /* branch/exchange thumb (bx) */
               || (sh == 0x2 && op1 == 1) /* bxj */
               || (sh == 0x3 && op1 == 1) /* blx */) {
                goto update_ret;
            }
...
//#define DEBUG_CC
#ifdef DEBUG_CC
static target_ulong track_pc[] = {
};
static bool is_track_pc(target_ulong pc) {
    int i;
    return true;
    for (i = 0; i < sizeof(track_pc)/sizeof(track_pc[0]); i++) {
        if (track_pc[i] == pc)
            return true;
    }
    return false;
...
static void disas_arm_insn(DisasContext *s, unsigned int insn, ARMInstruction *arm_instr)
{
    ...
    cond = insn >> 28;
	set_arm_instr_cc(arm_instr, cond); 
    if (cond == 0xf){
	...
	if ((insn & 0x0f900000) == 0x03000000) {
        ...
        case 0x1:
            if (op1 == 1) {
				...
				/* Since this is a branch and exchange instruction,
                   no need to save condition codes, */
                if (rm == 14 && cond == 0xe) {
                    //fprintf(stderr, "\t\t0x%x,\n", arm_instr->pc);
                    tcg_ctx->succ_define_cc = 3;
                }
				...
				set_arm_instr_opd_num(arm_instr, 2);
                set_arm_instr_opd_reg(arm_instr, 0, rd);
                set_arm_instr_opd_reg(arm_instr, 1, rm);
                set_arm_instr_opc(arm_instr, ARM_OPC_CLZ);
        ...
        /* set operand number based on opcode 
           1. 2 operands: tst, teq, cmp, cmn, mov, mvn
           2. 3 operands: and, eor, sub, rsb, add,
                          adc, sbc, rsc, bic */
        if (op1 == 0x08 || op1 == 0x09 || op1 == 0x0a || op1 == 0x0b ||
            op1 == 0x0d || op1 == 0x0f)
            set_arm_instr_opd_num(arm_instr, 2);
        else
            set_arm_instr_opd_num(arm_instr, 3);
        ...
        if (insn & (1 << 25)) {
			...
			set_arm_instr_opd_imm(arm_instr, arm_instr->opd_num - 1, val);
            if (logic_cc && shift) {
                gen_set_CF_bit31(tmp2);
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
            }
        } else {
            /* register */
            rm = (insn) & 0xf;
			
			set_arm_instr_opd_reg(arm_instr, arm_instr->opd_num - 1, rm);
            ...
            if (!(insn & (1 << 4))) {
			    ...
			    set_arm_instr_opd_reg_scale_imm(arm_instr, arm_instr->opd_num - 1, shiftop, shift);
				...
				set_arm_instr_opd_reg_scale_reg(arm_instr, arm_instr->opd_num - 1, shiftop, rs);
        ...
        if (op1 != 0x0f && op1 != 0x0d) {
			...
			set_arm_instr_opd_reg(arm_instr, arm_instr->opd_num - 2, rn);
		...
		if (arm_instr->opd_num > 2 || op1 == 0x0f || op1 == 0x0d)
            set_arm_instr_opd_reg(arm_instr, 0, rd);
		
        switch(op1) {
        case 0x00:
			
			set_arm_instr_opc(arm_instr, ARM_OPC_AND);
            tcg_gen_and_i32(tmp, tmp, tmp2);
            if (logic_cc) {
				
				set_arm_instr_opc(arm_instr, ARM_OPC_ANDS);
                gen_logic_CC(tmp);
			 	s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
            }
        ...
        case 0x01:/* eor */
			...
			set_arm_instr_opc(arm_instr, ARM_OPC_EOR);
            tcg_gen_xor_i32(tmp, tmp, tmp2);
            if (logic_cc) {
				set_arm_instr_opc(arm_instr, ARM_OPC_EORS);
                gen_logic_CC(tmp);
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
            }
        ...
        case 0x02:/* sub */
            if (set_cc && rd == 15) {
                ...
                if (set_cc) {
					...
					set_arm_instr_opc(arm_instr, ARM_OPC_SUBS);
                    gen_sub_CC(tmp, tmp, tmp2);
					s->cc_defined = true;
                    tcg_gen_movi_i32(cpu_CC_Source, 0);
					
                } else {
					set_arm_instr_opc(arm_instr, ARM_OPC_SUB);
        ...
        case 0x03:/* rsb */
            if (set_cc) {
				...
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
				set_arm_instr_opc(arm_instr, ARM_OPC_RSBS);
            } else {
                tcg_gen_sub_i32(tmp, tmp2, tmp);
				set_arm_instr_opc(arm_instr, ARM_OPC_RSB);
            }
            store_reg_bx(s, rd, tmp);
            break;
        case 0x04:/* add */
            if (set_cc) {
				...
				set_arm_instr_opc(arm_instr, ARM_OPC_ADDS);
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
            } else {
                tcg_gen_add_i32(tmp, tmp, tmp2);
				set_arm_instr_opc(arm_instr, ARM_OPC_ADD);
            }
            store_reg_bx(s, rd, tmp);
            break;
        case 0x05:/* adc */
            if (set_cc) {
				...
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
				set_arm_instr_opc(arm_instr, ARM_OPC_ADCS);
            } else {
                gen_add_carry(tmp, tmp, tmp2);
				set_arm_instr_opc(arm_instr, ARM_OPC_ADC);
            }
            store_reg_bx(s, rd, tmp);
            break;
        case 0x06:/* sbc */
            if (set_cc) {
				...
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
				set_arm_instr_opc(arm_instr, ARM_OPC_SBCS);
            } else {
                gen_sub_carry(tmp, tmp, tmp2);
				set_arm_instr_opc(arm_instr, ARM_OPC_SBC);
            }
            store_reg_bx(s, rd, tmp);
            break;
        case 0x07:/* rsc */
            if (set_cc) {
				...
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
				set_arm_instr_opc(arm_instr, ARM_OPC_RSCS);
            } else {
                gen_sub_carry(tmp, tmp2, tmp);
				set_arm_instr_opc(arm_instr, ARM_OPC_RSC);
            }
            store_reg_bx(s, rd, tmp);
            break;
        case 0x08:/* tst */
			set_arm_instr_opc(arm_instr, ARM_OPC_TST);
            if (set_cc) {
                tcg_gen_and_i32(tmp, tmp, tmp2);
                gen_logic_CC(tmp);
				s->cc_defined = true;
            ...
            break;
        case 0x09:/* teq */
			set_arm_instr_opc(arm_instr, ARM_OPC_TEQ);
            if (set_cc) {
                tcg_gen_xor_i32(tmp, tmp, tmp2);
                gen_logic_CC(tmp);
				s->cc_defined = true;
            ...
            break;
        case 0x0a:/* cmp */
			set_arm_instr_opc(arm_instr, ARM_OPC_CMP);
            if (set_cc) {
                gen_sub_CC(tmp, tmp, tmp2);
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
            ...
            break;
        case 0x0b:/* cmn */
			set_arm_instr_opc(arm_instr, ARM_OPC_CMN);
            if (set_cc) {
                gen_add_CC(tmp, tmp, tmp2);
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
            ...
            break;
        case 0x0c:/* orr */
			set_arm_instr_opc(arm_instr, ARM_OPC_ORR);
            tcg_gen_or_i32(tmp, tmp, tmp2);
            if (logic_cc) {
				set_arm_instr_opc(arm_instr, ARM_OPC_ORRS);
                gen_logic_CC(tmp);
				s->cc_defined = true;
                tcg_gen_movi_i32(cpu_CC_Source, 0);
            }
            ...
            break;
        case 0x0d:/* mov */
			set_arm_instr_opc(arm_instr, ARM_OPC_MOV);
            if (logic_cc && rd == 15) {
                ...
                if (logic_cc) {
					set_arm_instr_opc(arm_instr, ARM_OPC_MOVS);
                    gen_logic_CC(tmp2);
                    s->cc_defined = true;
					tcg_gen_movi_i32(cpu_CC_Source, 0);
                }
            ...
            break;
        case 0x0e:/* bic */
			set_arm_instr_opc(arm_instr, ARM_OPC_BIC);
            tcg_gen_andc_i32(tmp, tmp, tmp2);
            if (logic_cc) {
                gen_logic_CC(tmp);
				s->cc_defined = true;
				tcg_gen_movi_i32(cpu_CC_Source, 0);
                set_arm_instr_opc(arm_instr, ARM_OPC_BICS);
            }
            store_reg_bx(s, rd, tmp);
            break;
        default:
        case 0x0f:/* mvn */
			set_arm_instr_opc(arm_instr, ARM_OPC_MVN);
            tcg_gen_not_i32(tmp2, tmp2);
            if (logic_cc) {
                gen_logic_CC(tmp2);
				s->cc_defined = true;
				tcg_gen_movi_i32(cpu_CC_Source, 0);
                set_arm_instr_opc(arm_instr, ARM_OPC_MVNS);
            }
            store_reg_bx(s, rd, tmp2);
            break;
        }
        ...
        case 0x1:
            ...
            if (sh == 0) {
                if (op1 == 0x0) {
					...
					set_arm_instr_opd_num(arm_instr, 4);
                    set_arm_instr_opd_reg(arm_instr, 0, rd);
                    set_arm_instr_opd_reg(arm_instr, 1, rm);
                    set_arm_instr_opd_reg(arm_instr, 2, rs);
                    set_arm_instr_opd_reg(arm_instr, 3, rn);
                    ...
                    case 0: case 1: case 2: case 3: case 6:
                        ...
                        tcg_temp_free_i32(tmp2);
						set_arm_instr_opc(arm_instr, ARM_OPC_MUL);
                        if (insn & (1 << 22)) {
                            /* Subtract (mls) */
                            ARCH(6T2);
                            tmp2 = load_reg(s, rn);
                            ...
                            /* Add */
						 	set_arm_instr_opc(arm_instr, ARM_OPC_MLA);
                            tmp2 = load_reg(s, rn);
                            tcg_gen_add_i32(tmp, tmp, tmp2);
                            tcg_temp_free_i32(tmp2);
                        }
                    ...
                    case 12: case 13: case 14: case 15:
                        ...
                        if (insn & (1 << 22)) {
							...
							if (insn & (1 << 21))
                                set_arm_instr_opc(arm_instr, ARM_OPC_SMLAL);
                            else
                                set_arm_instr_opc(arm_instr, ARM_OPC_SMULL);
              
                        } else {
                            tcg_gen_mulu2_i32(tmp, tmp2, tmp, tmp2);
							 if (insn & (1 << 21))
                                set_arm_instr_opc(arm_instr, ARM_OPC_UMLAL);
                            else
                                set_arm_instr_opc(arm_instr, ARM_OPC_UMULL);
                        }
                ...
                set_arm_instr_opd_num(arm_instr, 2);
                set_arm_instr_opd_reg(arm_instr, 0, rd);
                set_arm_instr_opd_type(arm_instr, 1, ARM_OPD_TYPE_MEM);
                set_arm_instr_opd_mem_base(arm_instr, 1, rn);
                ...
                if (pbit) {
                   ...
                   gen_add_datah_offset(s, insn, 0, addr, &arm_instr->opd[1]);
                }
                address_offset = 0;
                ...
                if (doubleword) {
                    ...
                    switch (sh) {
                    case 1:/* ldrh */
						set_arm_instr_opc(arm_instr, ARM_OPC_LDRH);
                    ...
                    tcg_temp_free_i32(tmp);
					 set_arm_instr_opc(arm_instr, ARM_OPC_STRH);
                }
                ...
                if (!pbit) {
                    ...
                    gen_add_datah_offset(s, insn, address_offset, addr, &arm_instr->opd[1]);
                    store_reg(s, rn, addr);
					set_arm_instr_opd_mem_index_type(arm_instr, 1, ARM_MEM_INDEX_TYPE_POST);
                } else if (wbit) {
                    if (address_offset)
                        tcg_gen_addi_i32(addr, addr, address_offset);
                    store_reg(s, rn, addr);
					set_arm_instr_opd_mem_index_type(arm_instr, 1, ARM_MEM_INDEX_TYPE_PRE);
                } else {
                    tcg_temp_free_i32(addr);
                }
                if (load) {
        ...
        do_ldst:
            ...
            if (insn & (1 << 24))
                gen_add_data_offset(s, insn, tmp2, &(arm_instr->opd[1])); //gen_add_data_offset(s, insn, tmp2);
            if (insn & (1 << 20)) {
                /* load */
                tmp = tcg_temp_new_i32();
                if (insn & (1 << 22)) {
                    gen_aa32_ld8u_iss(s, tmp, tmp2, i, rd);
					set_arm_instr_opc(arm_instr, ARM_OPC_LDRB);
                } else {
                    gen_aa32_ld32u_iss(s, tmp, tmp2, i, rd);
					set_arm_instr_opc(arm_instr, ARM_OPC_LDR);
                }
				  #ifdef PROFILE_INDIRECT_BRANCH
                if (rd == 15){
                    profile_indirect();
                ...
                if (insn & (1 << 22)) {
					...
					set_arm_instr_opc(arm_instr, ARM_OPC_STRB);
                } else {
                    gen_aa32_st32_iss(s, tmp, tmp2, i, rd);
					set_arm_instr_opc(arm_instr, ARM_OPC_STR);
                }
                tcg_temp_free_i32(tmp);
            }
            if (!(insn & (1 << 24))) {
        ...
        case 0x09:
            {
				...
				if ((rn == 13) && (insn & (1 << 21))) {
									/* This is a push or pop, let's do rule translation */
									if (is_load) {
										if ((insn & (1 << 23)) && !(insn & (1 << 24)))
											set_arm_instr_opc(arm_instr, ARM_OPC_POP);
									} else {
										if (!(insn & (1 << 23)) && (insn & (1 << 24)))
											set_arm_instr_opc(arm_instr, ARM_OPC_PUSH);
									}
									set_arm_instr_opd_num(arm_instr, 1);
									set_arm_instr_opd_imm(arm_instr, 0, insn & 0xffff);
								}
                ...
                for (i = 0; i < 16; i++) {
                    if (insn & (1 << i)) {
						...
						if (i == 15)
                            set_arm_instr_opc(arm_instr, ARM_OPC_INVALID);
        ...
        case 0xb:
            {
				...
				if (op1 == 0xa)
					set_arm_instr_opc(arm_instr, ARM_OPC_B);
				else
					set_arm_instr_opc(arm_instr, ARM_OPC_BL);
                ...
                val += offset + 4;
				set_arm_instr_opd_num(arm_instr, 1);
                set_arm_instr_opd_imm(arm_instr, 0, val);
                gen_jmp(s, val);
				if (s->condjmp) {
                    int t1 = tb_define_cc(s, s->pc);
                    int t2 = tb_define_cc(s, val);
                    #define min(x, y) ((x) < (y) ? (x) : (y))
                    #define max(x, y) ((x) > (y) ? (x) : (y))
                    if (t1 == 3 && t2 == 3)
                        tcg_ctx->succ_define_cc = 3;
                    else if (t1 == 3 || t2 == 3)
                        tcg_ctx->succ_define_cc = min(t1, t2);
                    else
                        tcg_ctx->succ_define_cc = max(t1, t2);
                    #if 0
                    fprintf(stderr, "tb: %x\n\tfallthrough: %x  define cc: %d\n\ttarget: %x  define cc: %d\n",
                            s->tb->pc, s->pc, tb_define_cc(s, s->pc), val, tb_define_cc(s, val));
                    #endif
                }
            }
            break;
...
static void arm_post_translate_insn(DisasContext *dc)
{
	...
	next_page_start = dc->page_start + TARGET_PAGE_SIZE;
	
	if (dc->condjmp && !dc->cc_defined) {
		tcg_ctx->pc_use_predefined_cc[tcg_ctx->pc_use_predefined_cc_idx++] = dc->pc - 4;
		assert(tcg_ctx->pc_use_predefined_cc_idx < 20);
		#if 0
		fprintf(stderr, "-----------BB use cc but not define cc: %x\n", dc->pc - 4);
	...
	end_of_page = (dc->pc >= next_page_start) ||
		((dc->pc >= next_page_start - 3) && insn_crosses_page(dc->env, dc));

        if (dc->condjmp && !dc->base.is_jmp) {
			#if 1
            unsigned int insn = arm_ldl_code(dc->env, dc->pc - 4, dc->sctlr_b);
            unsigned int next_insn = arm_ldl_code(dc->env, dc->pc, dc->sctlr_b);
            if ((dc->prev_cond != 0xe &&
                (dc->prev_cond != (next_insn >> 28) /*|| ((next_insn >> 24) & 0xf) == 0xa*/
                 || insn_define_cc(insn))) || tcg_op_buf_full()
                 || end_of_page
				#ifdef DEBUG_CC
                 || is_track_pc((dc->tb)->pc)
				#endif
             	)
			#endif
...
static void arm_tr_translate_insn(DisasContextBase *dcbase, CPUState *cpu)
{
	...
	ARMInstruction *arm_instr = create_arm_instr(dcbase->tb, dc->pc, insn);
    ...
    dc->insn = insn;
    dc->pc += 4;
    disas_arm_insn(dc, insn,arm_instr);
...
void profile_block_dcount(TranslationBlock *tb);

#ifdef PROFILE_RULE_TRANSLATION
int static_inst_counter = 0;
#endif

#ifdef PROFILE_TB_HOTNESS
int static_tb_counter = 0;
#endif
...
TCGTemp *get_reg_tcg_temp(TCGContext *s, ARMRegister reg);
						  
TCGTemp *get_reg_tcg_temp(TCGContext *s, ARMRegister reg)
{
	 switch (reg) {
		case ARM_REG_CF:
			return &s->temps[GET_TCGV_I32(temp_idx(tcgv_i32_temp(cpu_CF)))];
		case ARM_REG_NF:
			return &s->temps[GET_TCGV_I32(temp_idx(tcgv_i32_temp(cpu_NF)))];
		case ARM_REG_VF:
			return &s->temps[GET_TCGV_I32(temp_idx(tcgv_i32_temp(cpu_VF)))];
		case ARM_REG_ZF:
			return &s->temps[GET_TCGV_I32(temp_idx(tcgv_i32_temp(cpu_ZF)))];
		default:
			 return &s->temps[GET_TCGV_I32(temp_idx(tcgv_i32_temp(cpu_R[reg-ARM_REG_R0])))];
	}
}
						  
unsigned int get_arm_insn_binary(TCGContext *s, target_ulong pc, TranslationBlock *tb);
unsigned int get_arm_insn_binary(TCGContext *s, target_ulong pc, TranslationBlock *tb)
{
	 return arm_ldl_code(s->env, pc, ARM_TBFLAG_SCTLR_B(tb->flags));
}
						  
#if defined(PROFILE_RULE_TRANSLATION)
   extern uint32_t static_inst_rule_counter;
#endif

#if defined(PROFILE_RULE_TRANSLATION_HOST)
   extern uint64_t dynamic_host_insn;
   extern uint64_t dynamic_host_rule_insn;
#endif

#if defined(PROFILE_RULE_TRANSLATION) || defined(PROFILE_RULE_TRANSLATION_HOST)
void dump_profiling_results(CPUARMState *);
						  
void dump_profiling_results(CPUARMState *env)
{
	 fprintf(stderr, "\n======================== Profiling Results =====================================\n");
#if defined(PROFILE_RULE_TRANSLATION)
	 fprintf(stderr, "Number of static guest instructions translatd: %u\n", static_inst_counter);
	//  fprintf(stderr, "Number of static guest instructions with matched translation rule: %u\n", static_inst_rule_counter);
	 fprintf(stderr, "Number of dynamic guest instructions emulated: %llu\n", env->total_dynamic_insts);
	 fprintf(stderr, "Number of dynamic guest instructions with matched tanslation rule: %llu\n", env->rule_dynamic_insts);
#endif
#if defined(PROFILE_INDIRECT_BRANCH)
	fprintf(stderr, "Number of indirect branch instructions: %llu\n", env->indirect_dynamic_insts);	 
#endif
						  
#if defined(PROFILE_RULE_TRANSLATION_HOST)
	fprintf(stderr, "Number of dynamic host instructions executed: %llu\n", dynamic_host_insn);
    fprintf(stderr, "Number of rule translated host instructions: %llu\n", dynamic_host_rule_insn);
#endif
	fprintf(stderr, "==================================================================================\n");
}
#endif
 ...
 void profile_block_dcount(TranslationBlock *tb)
{
							  ...
							  tcg_gen_movi_i32((TCGv_i32)base, (int32_t)tb);
						  
							  tcg_gen_ld_i64(tmp64, base, offsetof(TranslationBlock, dcount));
							  tcg_gen_addi_i64(tmp64, tmp64, 1);
							  tcg_gen_st_i64(tmp64, base, offsetof(TranslationBlock, dcount));
						  
							  tcg_gen_ld_i64(tmp64, cpu_env, offsetof(CPUARMState, total_dynamic_tbs));
							  tcg_gen_addi_i64(tmp64, tmp64, 1);
							  tcg_gen_st_i64(tmp64, cpu_env, offsetof(CPUARMState, total_dynamic_tbs));
						  
							  tcg_temp_free_i64(tmp64);
}
						  
void dump_tb_hotness(CPUARMState *env);
void dump_tb_hotness(CPUARMState *env) {
#if 1
							  int i, j;
							  for(i = 0; i < tcg_ctx.tb_ctx.nb_tbs; i++) {
								 uint64_t max = 0;
								 TranslationBlock *mtb = NULL;
								 for (j = 0; j < tcg_ctx.tb_ctx.nb_tbs; j++) {
									  TranslationBlock *tb = &tcg_ctx.tb_ctx.tbs[j];			
									  if (tb->dcount > 5000000 && tb->dcount > max && tb->pflag == 0) {
										  mtb = tb;
										  max = tb->dcount;
									  }
								  }
...
#endif
							  fprintf(stderr, "Number of static tbs: %d\n", static_tb_counter);
							  fprintf(stderr, "Number of dynamic tbs executed: %llu\n", env->total_dynamic_tbs);
}
#endif
						  
#ifdef DEBUG_RULE_TRANSLATION
	target_ulong debug_pc = 0;//x4ac28;
#endif
						  
#ifdef PROFILE_INDIRECT_BRANCH
void profile_indirect();
void profile_indirect(){
							  TCGv_i64 tmp64 = tcg_temp_new_i64();
						  
							  tcg_gen_ld_i64(tmp64, cpu_env, offsetof(CPUARMState, indirect_dynamic_insts));
							  tcg_gen_addi_i64(tmp64, tmp64, 1);
							  tcg_gen_st_i64(tmp64, cpu_env, offsetof(CPUARMState, indirect_dynamic_insts));
}
#endif
