...
#else
#error "Not support arch except ARM"
#endif
...
void cpu_gen_init(void)
{
    tcg_context_init(&tcg_init_ctx);
	 ...
	 /* Should be reloadable for retargetable DBT */
    arm_instr_buffer_init();
}
                              ...
                              uint32_t flags, int cflags)
{
 ...
 tb_overflow:
	...
	// init_rule_translation();
	   /* Perform translation rule match */
	   // if (tb->pc == 0x84a0)
	   // time_t start_time = time();
	   // match_translation_rule(tb);
	   // time_t end_time = time();
	   // global_match_time += end_time - start_time;
   #if 0
	   if (tb->num_rules_hit > 0)
	   {-
		   fprintf(stderr, "tb->pc: %x, tb->icount: %d, tb->num_insts_hit: %d\n",
				   tb->pc, tb->icount, tb->num_insts_match);
		   target_disas(stderr, cpu, tb->pc, tb->icount * 4, 0);
		   fprintf(stderr, "\n\n");
	   }
   #endif
    ...
    if (tb->pc == debug_pc)
    {
        fprintf(stderr, "======== tb->pc: %x, tb->tc_ptr: %p\n", tb->pc, tb->tc_ptr);
        disas(stderr, tb->tc_ptr, gen_code_size);
        sleep(20);
    }
#endif
