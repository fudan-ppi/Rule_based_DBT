...
#define MAX_RULE_RECORD_BUF_LEN 600
#define MAX_GUEST_INSTR_LEN 600
#define MAX_HOST_RULE_LEN 600
...
static const ARMOpcode opc_set[] = {
    ...
    [ARM_OPC_BIC]   = ARM_OPC_INVALID,
    [ARM_OPC_BICS]  = ARM_OPC_INVALID,
    ...
    [ARM_OPC_ADC]   = ARM_OPC_OP1,
    [ARM_OPC_SUB]   = ARM_OPC_INVALID,
    [ARM_OPC_SBC]   = ARM_OPC_INVALID,
    [ARM_OPC_RSC]   = ARM_OPC_INVALID,
    [ARM_OPC_ADDS]  = ARM_OPC_OP5,
    [ARM_OPC_ADCS]  = ARM_OPC_OP5,
    [ARM_OPC_SUBS]  = ARM_OPC_INVALID,
    [ARM_OPC_SBCS]  = ARM_OPC_INVALID,
    [ARM_OPC_RSCS]  = ARM_OPC_INVALID,
    ...
    [ARM_OPC_RSB]   = ARM_OPC_INVALID,
    [ARM_OPC_RSBS]  = ARM_OPC_INVALID,
...
static const X86Opcode ARM2X86[] = {
    ...
    [ARM_OPC_CMP]   = X86_OPC_INVALID,
    [ARM_OPC_CMN]   = X86_OPC_INVALID,
    [ARM_OPC_B]     = X86_OPC_INVALID,
    [ARM_OPC_BL]    = X86_OPC_INVALID,
...
//                                    int icount, bool update_cc, bool save_cc)
// {
...
static bool match_scale(ARMOperandScale *gscale, ARMOperandScale *rscale)
{
    ...
    if (gscale->type == ARM_OPD_SCALE_TYPE_IMM) {
...
static bool match_rule_internal(ARMInstruction *instr, TranslationRule *rule, TranslationBlock *tb)
{
    ...
    // if ((p_rule_instr->opc == ARM_OPC_OP1)){
    //     print_arm_instr(p_rule_instr);
    //         // print_arm_instr(p_guest_instr);
    //     }
   X86Instruction *x86_temp = rule->x86_host;

  	while(x86_temp){
	
		if (x86_temp->opc == X86_OPC_INCL||
			x86_temp->opc == X86_OPC_DECL||
            x86_temp->opc == X86_OPC_LEAL){
			return false;
		}
		x86_temp = x86_temp ->next;
  	}
    ...
    while(p_rule_instr) {
        ...
        // if ( ((p_rule_instr->opc != p_guest_instr->opc) && (opc_set[p_guest_instr->opc] != p_rule_instr->opc)) ||
        //     p_rule_instr->opd_num != p_guest_instr->opd_num) {
        // if (p_rule_instr->opc != p_guest_instr->opc ||
        //     p_rule_instr->opd_num != p_guest_instr->opd_num) {
            #if 0
            if (instr->pc == 0x17ae0) {
        ...
        if ((p_guest_instr->opc == ARM_OPC_STRH) || (p_guest_instr->opc == ARM_OPC_LDRH)){
        ...
        for(i = 0; i < p_rule_instr->opd_num; i++) {
            ...
            // if ((rule->guest_instr_num == 1) && (p_rule_instr->opc == ARM_OPC_OP1) && (p_guest_instr->opc == ARM_OPC_AND)){
            //     fprintf(stderr, "Stage_01\n");
            // }


            // if (strstr(rule->intermediate_regs, get_arm_reg_str(p_rule_instr->opd[i].content.reg.num)) && used_following(p_guest_instr, i, rule->guest_instr_num - j)){
            //     return false;
            // }
        }
    ...
    // if ((rule->index == 112) && (rule->arm_guest->opc == ARM_OPC_OP1) && (instr->opc == ARM_OPC_AND)){
        ...
        // for(i = max; i > 0; i--) {
            ...
            if (cur_rule) {
                ...
                *print_block = 0;
                ...
                // check the parameterized opcode
                // ARMInstruction *p_rule_instr = cur_rule->arm_guest;
                // ARMInstruction *p_guest_instr = cur_head;
                // X86Instruction *p_rule_host = cur_rule->x86_host;

                // // fprintf(stderr, "==============Generating parametered host\n");
                // TranslationRule *n_rule = &record_host_buf[record_host_buf_index++];
                // n_rule->index = cur_rule->index;
                // n_rule->arm_guest = cur_rule->arm_guest;
                // n_rule->x86_host = &record_host_instr_buf[record_host_instr_buf_index++];
                // X86Instruction *n_rule_host = n_rule->x86_host;
                // X86Instruction *pre_n_rule_host = NULL;

                // // fprintf(stderr, "==============Copying host instruction\n");
                // while (p_rule_host){
                //     memcpy(n_rule_host, p_rule_host, sizeof(*p_rule_host));
                //     n_rule_host->next = &record_host_instr_buf[record_host_instr_buf_index++];
                //     pre_n_rule_host = n_rule_host;
                //     n_rule_host = n_rule_host->next;
                //     p_rule_host = p_rule_host->next;
                // }

                // pre_n_rule_host->next = NULL;
                // --record_host_instr_buf_index;
                // n_rule_host = n_rule->x86_host;
                // pre_n_rule_host = NULL;
                // // fprintf(stderr, "==============Replacing parameterized opcodes and operands\n");
                // //generate opcode and operands for parameterized instructions
                // while (p_rule_instr){
                //     // fprintf(stderr, "==============P_rule opcode: %d  %d\n", p_rule_instr->opc, ARM_OPC_OP1);
                //     #ifdef PROFILE_PARAMETER_RULE_TRANSLATION
                //         print_arm_instr(p_guest_instr);
                //     #endif
                //     if ((p_rule_instr->opc >= ARM_OPC_OP1) && (p_rule_instr->opc < ARM_OPC_END)){
                //         while (n_rule_host && (n_rule_host->opc != (p_rule_instr->opc+18))){
                //             pre_n_rule_host = n_rule_host;
                //             n_rule_host = n_rule_host->next;
                //         }
                //         //set opcode
                ...
                //         if (n_rule_host){
                //             n_rule_host->opc = ARM2X86[p_guest_instr->opc];
                //         }
                //         else
                //         {
                //             goto para_fault;
                ...
                //         // fprintf(stderr, "==============Replacing opcode: %d\n", n_rule_host->opc);
                //         //set operands
                //         /*if (n_rule_host->opd_num == 0){
                //             if (p_rule_instr->opc == ARM_OPC_OP1){
                //                 X86Instruction *temp = al_instr_match(n_rule_host, p_guest_instr);
                //                 if (!temp)
                //                     goto para_fault;
                //                 if (pre_n_rule_host)
                //                     pre_n_rule_host->next = temp;
                //                 else n_rule->x86_host = temp;         
                //             }
                //             else if (!topd_instr_opd_match(n_rule_host, p_guest_instr)){
                //                 // fprintf(stderr, "==============Not matched: %d\n", cur_rule->index);
                //                 goto para_fault;
                //             }
                //         }*/
                //     }
                ...
                // }
                // #ifdef PROFILE_PARAMETER_RULE_TRANSLATION
                //     print_x86_instr_seq(n_rule->x86_host);
                // #endif
                // match_insts += i;
                // // fprintf(stderr, "==============Adding rule record\n");
                // add_rule_record(n_rule, cur_head->pc, target_pc, i, is_update_cc(cur_head, i), is_save_cc(cur_head, i));
                
                // // add the match_counter
                // ++(cur_rule->match_counter);
                // TranslationRule *prev_rule = cur_rule->prev;
                // //re-order the link
                // //find the last rule that counter > cur_rule
                // while (prev_rule && (cur_rule->match_counter > prev_rule->match_counter)){
                //     prev_rule = prev_rule->prev;
                // }
                // // find one rule that match_count > cur_rule
                // if (prev_rule){
                //     // remove cur_rule
                //     cur_rule->prev->next = cur_rule->next;
                //     if (cur_rule->next)
                //         cur_rule->next->prev = cur_rule->prev;
                //     // cur->next = prev->next
                //     if (prev_rule->next)
                //         prev_rule->next->prev = cur_rule;
                //     cur_rule->next = prev_rule->next;
                //     // cur = prev_next;
                //     cur_rule->prev = prev_rule;
                //     prev_rule->next = cur_rule;
                // }
                // else if (cur_rule->prev){
                //     //not find any rule that count > cur_rule
                //     //cur_rule will be the head of link
                //     cur_rule->prev->next = cur_rule->next;
                //     if (cur_rule->next)
                //         cur_rule->next->prev = cur_rule->prev;
                    
                //     //move cur_rule to head
                //     cur_rule->next = rule_table[hindex];
                //     rule_table[hindex]->prev = cur_rule;
                //     cur_rule->prev = NULL;
                //     rule_table[hindex] = cur_rule;
                // }
                break; 
            }
            // para_fault:
